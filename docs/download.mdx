...
<Steps>
  <Step title="Download">
    <a href="https://github.com/Benedict-Carling/eyeryhthm/releases/latest" style={{display: 'inline-flex', alignItems: 'center', gap: '10px', padding: '10px 18px', borderRadius: '6px', backgroundColor: '#3E63DD', color: '#fff', textDecoration: 'none', fontWeight: 500, fontSize: '14px'}}>
      Download for macOS (.dmg)
    </a>

    Look for `EyeRhythm-x.x.x-universal.dmg` in the release assets.
  </Step>
  ...
  <Step title="First launch (important)">
    Double-click the app to open it. macOS will show a security warning saying **"Apple could not verify"** the application.

    <Frame>
      <img src="/images/macos-security-warning.png" alt="macOS security warning dialog" />
    </Frame>

    Click **"Done"** to dismiss this popup.
  </Step>
  <Step title="Allow in Privacy & Security">
    Open **System Settings > Privacy & Security**, scroll down to find the message about EyeRhythm being blocked, and click **"Open Anyway"**.

    <Frame>
      <img src="/images/macos-privacy-security.png" alt="macOS Privacy & Security settings" />
    </Frame>
  </Step>
...
<Note title="macOS Security Notice">
  Since EyeRhythm is not distributed through the Mac App Store, macOS requires you to manually approve the app in Privacy & Security settings. You only need to do this once. After the first launch, the app will open normally.
</Note>
...
<Steps>
  <Step title="Download">
    ...
    Look for `EyeRhythm.Setup.x.x.x.exe` in the release assets.
  </Step>
...
<Steps>
  <Step title="Download">
    ...
    Look for `EyeRhythm-x.x.x.AppImage` in the release assets.
  </Step>
...
<Steps>
  <Step title="Download">
    ...
    Look for `eyeryhthm_x.x.x_amd64.deb` in the release assets.
  </Step>
  <Step title="Install the package">
    Double-click the `.deb` file to open in Software Center, or install via terminal:
    ```bash
    sudo dpkg -i eyeryhthm_*.deb
    ```
  </Step>
